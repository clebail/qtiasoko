...
#### 🎉 Session des 2026-08-09 au 2026-08-11 — LA SÉRIE `ordre-look` : le 26 tombe, et TROIS MURS MÉMOIRE mesurés

**Contexte.** Le régime `ordre-look` (§6.2, `5aeae01`) change l'ordre de **9 niveaux sur 35** — 10, 12,
14, 25, 26, 27, 29, 31, 32. Les 26 autres sont bit-à-bit identiques, donc équivalents par construction :
**il n'y avait que ces neuf-là à relancer**, et c'est le diff d'ordre qui l'a établi, pas un solve.
Le 12 et le 27 étaient déjà tombés ; le 10 et le 32 étaient mesurés. Restaient **14, 25, 26, 29, 31**.

**RÉSULTAT : un sur cinq.**

| niveau | buts | verdict | ce qui l'a arrêté |
|---|---|---|---|
| **26** | 13 | ✅ **RÉSOLU** — 103 640 691 états, 197 poussées, 639 coups | — |
| 14 | 18 | diverge | `file/vus = 88 %`, pente +2 600, 139 M états |
| 25 | 19 | mur mémoire | palier de `f` **non franchi en 45 M dépilements** |
| 31 | 20 | mur mémoire | 188,6 M états — **reproduit le point d'arrêt de juillet** |
| 29 | 16 | mur mémoire | **213,7 M états, 14 Go, pic 15 Go** |

---

**🎉 LE 26 — le plus gros solve du projet.** 103,6 M états contre 6,6 M pour le 32 qui détenait le
record, ~7 Go de pic, ~1 h 30 de CPU. Le corral y a dépensé **362 M états de sous-solve** pour 2,65 M
configurations distinctes (amorti **×25,8** par le cache) et élagué **26,8 M enfilages** : le
sous-solve coûte **3,5 fois** la recherche principale. Sans lui, on ne sait pas s'il tombait.

> ⚠️ **AUCUN BUDGET DE DEUX MINUTES NE POUVAIT LE VOIR VENIR.** Tous les profilages du §6.6 tournaient
> à 120 s ; le 26 demande 103,6 M états et une heure et demie. Il était classé **Groupe B** (7/13,
> pente +1080). Le 12, lui, était **Groupe A** et il est tombé la semaine dernière. **La partition
> Groupe A / Groupe B perd son dernier point d'appui** — le §6.6 la présentait comme « ce qui SURVIT »
> du profilage après l'élimination des records morts, de la progression bornée et de la pente. Il ne
> reste plus rien de ce profilage.

**⚠️ `ordre-look` N'AGIT PAS PARTOUT OÙ IL CHANGE L'ORDRE.** Le 31 en est la preuve : son ordre est
**différent** sous le régime, et sa trajectoire est **identique à celle de juillet au dépilement près**.

| | 2026-07-29 (`coupl-plongeon`) | 2026-08-10 (`ordre-look`) |
|---|---|---|
| max | 16/20 | 16/20 |
| états vus | 188 M | 188,6 M |
| front | `f 248 h(reste) 220`, `g ≈ 28` à 107 M dépilements | **identique** |

Deux régimes, deux ordres, un seul et même point d'arrêt. C'est une donnée pour la colonne « famille »
du §6.6, toujours vide pour ce levier : il fait tomber le 12, le 26 et le 27, il est **neutre** sur le
31, et il **casse** le 32.

---

**✅ LE MODÈLE MÉMOIRE DU §6.5, VALIDÉ SUR QUATRE TAILLES.** Il était marqué *« calculés, pas mesurés :
confirmer par la RSS réelle avant de dimensionner quoi que ce soit »* depuis le 2026-07-24. Mesuré à
`footprint` (jamais `ps rss`, qui ment sur macOS) :

| niveau | buts | Mo par million d'états |
|---|---|---|
| 26 | 13 | **61** (le §6.5 prédisait 63 pour le 11 — écart 3 %) |
| 29 | 16 | **65** |
| 25 | 19 | **85** |
| 31 | 20 | **88** |

Croissance régulière avec le nombre de caisses, exactement ce que le §6.5 énonce (*« l'arène domine,
et son coût par clé croît avec le nombre de caisses »*). ⚠️ **Le coût par état BAISSE en cours de
run** — 131 Mo/M sur le 31 à 9,9 M états, 88 à 34,6 M : les arènes allouent par blocs, donc toute
extrapolation faite tôt surestime. J'ai annoncé sur cette base un « plafond sous 115 M » qui était faux
d'un facteur deux.

> 🎯 **LE PLAFOND RÉEL DE LA MACHINE, ENFIN MESURÉ : plus de 213 M états.** Le §6.5 l'estimait à
> ~150 M depuis juillet. Le 29 a atteint **213,7 M états pour 14 Go** (pic 15 Go, 18 Go de RAM,
> swap 4,8/6,1 Go) avant de ne plus rien produire. **Trois des cinq niveaux de la série sont morts de
> la MÉMOIRE, pas du temps** — c'est le cas de figure que le §6.5 attendait depuis sa réouverture, et
> il est maintenant le mode d'échec DOMINANT.

**⚠️ LE PIÈGE D'UNITÉ QUI M'A FAIT PUBLIER « 14 Mo ».** `footprint` bascule de `MB` à `GB` au-delà du
giga ; mon filtre cherchait `MB` et a rendu un chiffre absurde — 14 Mo pour 213 M états — que j'ai
failli commenter. **Un ordre de grandeur aberrant est un bug de parseur avant d'être une découverte**,
même leçon que le `AUCUNE 0` du 2026-08-07.

**⚠️ ET `ps -o etime=` EST PIRE QUE LE MURAL.** J'ai annoncé « 13 h 36 » sur le 29 ; le CPU réel était
de **17 minutes** — le reste était une nuit de veille, relevée par l'utilisateur. Le §1 proscrit le
mural au profit du CPU mais ne nomme pas `etime`, qui peut surestimer d'un facteur **50**. Pour un
processus déjà lancé : **`ps -o time=`, jamais `-o etime=`**.

---

**CE QUE LES ÉCHECS APPRENNENT, chiffré plutôt que « ne termine pas » :**
- **25** — `f` figé à 376 pendant **45 M dépilements** pour un budget mémoire restant de 48 M. Ce
  n'est pas un pronostic sur la difficulté, c'est une soustraction : **une unité de travail coûte plus
  que ce qui reste**. Sa progression était par ailleurs de `max 2/19` à 15 M dépilements, quand le 26
  atteignait 7/13 en **14 000**.
- **29** — `max` figé à **10/16 pendant 106 M dépilements**, et son record 10/16 réfuté par le plongeon
  **en 15 états** sur un budget de 719 000. Espace épuisé, branche morte. Le plongeon ne s'y déclenche
  quasiment jamais : **4 fois en 36 M dépilements, 0,0013 % du travail** (le 31 : 16 fois, 2,9 %).
- **14** — le seul qui diverge au lieu de manquer de place. Prédiction écrite **avant** le run et
  vérifiée : il lui manque aussi le stockage (cf. [journal-hybride.md](journal-hybride.md), 2026-08-09).

**⚠️ UN SIGNAL QUI RÉSISTE, ET DEUX QUI NE RÉSISTENT PAS.**
- **Résiste** : `rangees` AU FRONT contre `max`. Sur le 31, `max 16/20` avec `rangees 0` — le max était
  un **pic isolé** d'une branche lointaine. Sur le 29, `rangees 8-9` avec `max 9-10` — le front EST au
  niveau du record. La distinction sépare proprement « la recherche travaille là » de « elle y est
  passée une fois ».
- **Ne résiste pas** : le **ratio file/vus**. Je l'avais présenté comme discriminant après le 26 (22 %)
  contre le 14 (88 %). Faux : le 26 était à **65 %** au stade où le 25 était à 56 %, et c'est le 26 qui
  a gagné. Il ne se lit qu'en fin de course, quand il ne sert plus à rien.
- **Ne résiste pas** : la **pente de la file**, déjà réfutée au §6.6 et rien ici ne la réhabilite.

**Reste ouvert :**
- [ ] **La série est close, sauf le 32** — seul niveau que le régime CASSE, et son verrou est le stock,
  pas l'ordre (deux caisses identifiées, journal-hybride 2026-08-09).
- [ ] **La mémoire est devenue le mur dominant.** Trois échecs sur cinq. Le §6.5 liste l'arène comme
  premier poste à attaquer ; jamais fait, et ce n'était pas urgent tant que le mur était le temps.
  **Il ne l'est plus.**
