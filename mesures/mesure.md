...
| `rejeu <niveau.xsb> <journal.txt>` | **La partie humaine tient-elle, et le solveur la couperait-il ?** Rejoue la DERNIÈRE partie d'un journal hybride coup par coup et interroge le moteur à chaque état : sur une partie GAGNÉE, toute détection de défaite est un faux positif PROUVÉ (protocole du juge FP). Vérifie aussi la coupe d'ordre `remplissageOrdonne`. ⚠️ Il ne juge que les états TRAVERSÉS ; il ne dit pas si le solveur aurait généré les mêmes coups. |
| `zonembut [<niv\|fichier.xsb>]` | **Où est la salle de buts, et par où y entre-t-on ?** Un `.xsb` par ZONE D'EMBUT — l'enclos qui enferme un bloc de buts adjacents, avec ses murs et ses portes. Sans argument : les 32 niveaux. Appelle `Game::zonesEmbut()`, ne calcule rien lui-même. Quatre étages : salles de buts → goulots et portes doubles → absorption des culs-de-sac → **resserrage par coupe** (1 ou 2 cases, pas forcément voisines) quand rien ne sépare la salle du reste. ⚠️ **Relire la sortie en IMAGES** (`mesures/image` sur chaque `.xsb`, puis une planche contact) : c'est ce qui a réfuté un mécanisme que le jeu de validation à la main avait pourtant approuvé. `SEUIL=n` règle la taille max d'un cul-de-sac avalé (défaut 4, balayé : 4/5/6 donnent le même résultat). ⚠️ La sortie ne contient QUE les murs, les embuts et les caisses **déjà posées** sur un embut (`*`) : les autres caisses et le joueur sont retirés, la zone décrit une géométrie, pas une partie. ⚠️ Une porte est un TROU dans le mur, jamais un caractère — `Level::load` traduit tout caractère inconnu en case vide, donc un marqueur ferait de la sortie un faux `.xsb`. |
