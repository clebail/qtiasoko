...
int main(int argc, char** argv) {
    ...
    if (argc > 5 && (QString(argv[5]) == "livraison" || QString(argv[5]) == "couplage"
                     || QString(argv[5]) == "corral")) {
        const QString quel = argv[5];
        // 'corral' : le test bon marché est corralUnitaireMort() (balayage COMPLET
        // — on interroge des états échantillonnés quelconques, sans « caisse qui
        // vient d'arriver » à exploiter). La ligne « capture » donne alors la
        // FRÉQUENCE cherchée : la part des états morts qui présentent le motif.
        auto testMort = [&](const Game& gg){
            if (quel == "corral")   return gg.corralUnitaireMort();
            if (quel == "couplage") return livraisonMorteCouplage(gg);
            return livraisonMorte(gg);
        };
        const bool collMacro = !(argc > 6 && QString(argv[6]) == "opt");
        printf("niveau %d — %s : collecte %d etats (%s), classif oracle vs test\n",
               num, quel.toUpper().toLocal8Bit().constData(), plafond, collMacro ? "macro" : "opt");
        fflush(stdout);
        limiteDepilements() = plafond;
        etatsDeveloppes().clear();
        ...
        printf("\n===== NIVEAU %d — TEST %s vs ORACLE (%d etats) =====\n",
               num, quel.toUpper().toLocal8Bit().constData(), n);
        const int mort = TP + FN, sol = FP + TN;
        printf("  MORTS oracle   : %3d  -> test attrape %3d  (capture %2.0f %%)  <- frequence du motif\n",
