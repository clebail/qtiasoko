...
**19 sessions.** Verdict en tête : ✅ acquis · ❌ réfuté · ⏸️ sans verdict ·
...
| ✅ | 2026-08-07 | LA LOI DE L'ORDRE A ENFIN UN GAIN : ÷2,98 sur le 12, ZÉRO sur le 27 |
| 🎯 | 2026-08-09 | LE STOCK SE DÉDOUBLE : « garder pour plus tard » n'est PAS du stockage |
...
#### ✅ Session du 2026-08-07 — LA LOI DE L'ORDRE A ENFIN UN GAIN : ÷2,98 sur le 12, ZÉRO sur le 27

**C'est l'item que la session du 2026-08-03 laissait ouvert, et que celle du 2026-08-04 répétait** :
*« Le GAIN de la loi n'est mesuré nulle part […] La loi est validée en JUSTESSE, pas en RENDEMENT. »*
Les seuls chiffres existants — 247 prunes de gel sur le 7 (−2,4 %), 860 sur le 9, file ÷1,3 sur le 16 —
portaient sur des niveaux hors sujet. Le protocole convenu était : *« mesurer le gain sur 12/14/15/16
avec leur ordre humain injecté »*. Fait, sur les deux niveaux qui ont un ordre injecté au dépôt.

**Protocole** : `bench <niv> coupl-plongeon` contre `bench <niv> loi`, **même binaire, même commit**,
lancés depuis la RACINE pour que `ordre_niveau_00{12,27}.txt` s'applique (vérifié bruyamment :
`[ORDRE_FICHIER] lecture de …` puis `[ORDRE_HUMAIN] ordre injecté : 15 buts`). Une seule variable, le
régime. macOS arm64.

| niveau | régime | états | poussées | coups | pic mémoire |
|---|---|---|---|---|---|
| **12** | coupl-plongeon | 2 097 523 | 212 | 873 | 541,6 Mo |
| **12** | **`loi`** | **704 591** | 212 | 873 | **263,1 Mo** |
| 27 | coupl-plongeon | 332 359 | 363 | 1 536 | 83,8 Mo |
| 27 | **`loi`** | **332 359** | 363 | 1 536 | 79,7 Mo |

> 🎯 **Sur le 12 : ÷2,98 en états, ÷2,06 en mémoire, à POUSSÉES IDENTIQUES.** Premier gain net mesuré
> pour la loi de l'ordre, et il tombe exactement là où elle avait été conçue — un niveau dont l'ordre
> est le verrou, joué avec son ordre humain.
>
> **Sur le 27 : ZÉRO, à l'état près.** 332 359 des deux côtés, pas un état d'écart. Ça **confirme la
> nature établie le 2026-08-03** — la loi est un test de COHÉRENCE entre un ordre et une partie, pas
> un test d'ordre absolu : quand le solveur suit déjà l'ordre injecté de près, elle n'a rien à couper.

**⚠️ LE CORRECTIF DE LA MACRO DU JOUR EST NEUTRE ICI** (témoin contre le binaire `1308642`,
reconstruit par `git worktree`, même ordre injecté) :

| niveau | binaire `1308642` | binaire du jour | écart |
|---|---|---|---|
| 27 | 332 359 / 363 / 1 536 | 332 359 / 363 / 1 536 | **identique** |
| 12 | 2 097 527 / 212 / 873 | 2 097 523 / 212 / 873 | **4 états sur 2,1 M** |

C'est cohérent avec le journal-macro du jour : le demi-tour rendu jouable mord sur les niveaux où la
macro TOURNE (17 en ÷1,33, 9, 2, 4), pas sur ceux où elle ne démarre pas. **Le verrou du 12 et du 27
est l'ordre, et il le reste.**

**⚠️ UNE FAUSSE ALERTE, ET SA LEÇON.** J'ai d'abord lu un écart « 365 → 363 poussées » sur le 27 et
l'ai attribué au correctif de la macro. **Il n'y avait aucun écart** : 365 est le compte de la PARTIE
JOUÉE À LA MAIN, écrit une ligne au-dessus du relevé du solveur dans la session du 2026-08-06, qui
enregistrait déjà `332 359 dépilements, 363 poussées`. Ma mesure reproduit ce relevé **exactement**.
**Deux nombres voisins dans un même paragraphe ne décrivent pas forcément le même objet** — ici l'un
est humain, l'autre machine, et rien dans leur voisinage ne le signale. Le témoin l'a tranché en deux
minutes, ce qui est la seule raison pour laquelle la fausse attribution n'a pas survécu.

**⚠️ NI L'UN NI L'AUTRE N'EST ÉLIGIBLE À [scores.md](scores.md)** — les deux solves reposent sur un
fichier d'ordre injecté, donc ne sont pas reproductibles avec le binaire par défaut. Le journal le
disait déjà pour le 27 le 2026-08-06 ; **ça vaut identiquement pour le 12**. Ce qui est prouvé, c'est
que l'ordre est le verrou ; pas que le solveur sache résoudre ces niveaux.

**Reste ouvert :**
- [ ] **POURQUOI le 12 et pas le 27 ?** La loi entre au tableau des leviers du §6.6 comme le seul dont
  la colonne « prédicteur » est vide. Les stats `[LOI]` partent avec la jauge (enfilages, PRUNES, gel
  hors tour, balayages) : les relever sur les deux runs dirait si le 27 déclenche peu de prunes ou
  beaucoup de prunes sans effet. **Non fait** — c'est la mesure la moins chère de la liste.
- [ ] **14, 15 et 16 n'ont pas d'ordre injecté au dépôt.** Le 14 et le 15 en ont un, extrait le
  2026-08-03 (leurs faux positifs passaient de 147 et 151 à 0) ; il n'a jamais été posé en fichier.
  C'est le complément direct de cette mesure, et le protocole du 2026-08-03 les nommait.
- [ ] **La contiguïté de run** reste ce qui rendrait ces deux résultats reproductibles SANS fichier.
  Les deux motifs sont maintenant localisés : entrelacement de deux colonnes parallèles sur le 12
  (le saut du rang 3, 13,7 → 15,5), colonnes contre rangées sur le 27 (le rang 1, 6,4 → 6,3 au lieu
  de 5,4).

#### 🎯 Session du 2026-08-09 — LE STOCK SE DÉDOUBLE : « garder pour plus tard » n'est PAS du stockage

**Distinction posée par l'utilisateur**, en réponse à une mesure où je les confondais :

> *« Moi je vois deux notions, le stockage comme pour le 16 et la seconde caisse du niveau 32. Et le
> "cette caisse ne gêne en rien, je la garde pour plus tard" avec la première caisse du 32 et les
> caisses en 9,2 et 11,2 du 14. »*

Le projet parlait de « stock » depuis le 2026-08-05 (*« le stock reste non modélisé »*, *« la
difficulté est en aval du stock »*) sans jamais le chiffrer ni s'apercevoir qu'il recouvrait **deux
gestes opposés** : ne rien faire, et faire un détour coûteux.

**LA MAILLE QUI LES SÉPARE** — `mesures/attente.py`, neuf : *la plus longue immobilité d'une caisse
**sur une case qui n'est pas un but**, en % de la partie*. Puis un seul critère de partage :
**attend-elle là où elle a commencé, ou là où on l'a mise ?**

| niveau | **sur place** (on n'y a pas touché) | **déplacée** (stockage) |
|---|---|---|
| **14** | (15,7) **97 %** · (11,2) **94 %** · (9,2) **87 %** · (3,3) 80 % | 65 · 59 · 34 · 19 % |
| **16** | 22 · 13 · 10 · 8 % | (4,11)→(4,12) **90 %** · (5,11)→(4,11) **89 %** · (5,8)→(5,10) **87 %** |
| 32 | (7,12) 57 % · (8,2) 33 % | (2,4)→(3,4) **93 %** · (10,5)→(10,6) 67 % · (10,7)→(10,8) 52 % |
| 13 | (2,10) 96 % · (3,9) 80 % · (2,8) 80 % | (2,6)→(2,7) 87 % · (10,7)→(10,2) 67 % |
| 12 | (6,7) 96 % · (5,7) 91 % · (7,7) 74 % | (5,5)→(4,5) 84 % · (6,4)→(4,4) 78 % |

> 🎯 **LES DEUX NIVEAUX DE RÉFÉRENCE SORTENT AUX DEUX EXTRÊMES, SANS QU'ON LE LEUR DEMANDE.** Le **14**,
> que l'utilisateur cite pour « garder pour plus tard », est dominé par les attentes **sur place** ;
> le **16**, son niveau de stockage depuis le 2026-08-05, n'a QUE des attentes **déplacées**. La maille
> n'a pas été ajustée pour ça — c'est le premier tirage qui l'a donné.

**LE 32 DONNE LES DEUX SPÉCIMENS DANS LA MÊME PARTIE**, suivis caisse par caisse (567 coups) :

| caisse | comportement |
|---|---|
| partie de **(2,4)** → (4,6) | dans la salle haut-gauche **des coups 1 à 534**, immobile en (3,4) **93 %** |
| partie de **(8,4)** → (4,7) | vient de l'autre bout du plateau, **monte** dans la salle aux coups **442-522**, puis **redescend** |

La seconde est du stockage caractérisé : un détour de 80 coups vers une case qui n'est pas son but.
La première n'a bougé qu'au **coup 1**, et ce coup est **forcé** : (1,4) est la seule case du plateau
à n'avoir qu'**un** voisin libre, le joueur y démarre, et sa sortie est occupée par cette caisse.

**⚠️ DEUX FILTRES, CHACUN AJOUTÉ APRÈS UNE LECTURE FAUSSE — c'est la partie instructive :**
1. **« pas un but »** : sans lui, la caisse (4,4) du 32 sortait en tête à « immobile 96 % ». Elle est
   en fait **livrée au coup 22 et jamais retouchée**. *Attendre sur son but, ce n'est pas attendre,
   c'est avoir fini.* Un compteur d'immobilité mesure la fin de partie si on ne l'en empêche pas.
2. **« sur place ou ailleurs »** : c'est ce filtre-là qui porte toute la distinction. Sans lui les
   deux notions se ressemblent — même durée, même allure.

**⚠️ ET J'AI CHERCHÉ LA SALLE AU MAUVAIS ENDROIT.** L'utilisateur a d'abord dit « en haut à droite »,
puis s'est corrigé en « en haut à gauche ». J'avais entre-temps établi que la poche haut-droite
(7,1)-(8,1) était faite de **cases mortes** — aucune caisse ne peut y être posée, les quatre appuis
sont des murs. La bonne lecture était accessible sans poser la question : **la mesure d'occupation
désignait déjà (3,4) à 93 %**, dans la salle de gauche, et je ne l'ai pas vue parce que je cherchais
à droite. *Quand une mesure et un énoncé se contredisent, relire la mesure avant de chercher la
géométrie.*

**CE QUE ÇA DIT DU SOLVEUR — deux trous, d'inégale difficulté :**
- **« Garder pour plus tard »** est le plus abordable : la caisse n'est sur le trajet de personne et
  ne bloque pas le joueur, donc **rien ne force à y toucher**. Mais le régime d'engagement fait
  l'inverse par construction — il ne génère que les macros qui envoient une caisse vers le but actif,
  donc il essaie en permanence de livrer celle qu'il faudrait laisser. **Aucune notion de report.**
- **Le stockage** est plus dur : il faut décider d'amener une caisse sur une case qui **n'est pas son
  but**, l'y laisser, et revenir. La macro ne sait pas l'exprimer, elle ne va qu'au but actif ; les
  poussées simples le font, mais à l'aveugle.

**État du code** : `mesures/attente.py` (neuf, rapatrié dans `mesures/` le jour même — la règle du §1
avait été violée trois fois : `juge_loi.py`, `corpus_ordre.py`, les scripts de dépouillement du
2026-08-06). Réutilise le parseur de `taches.py`, aucun second parseur écrit. **Rien dans le solveur.**

**Reste ouvert :**
- [ ] **Ne pas compter comme déplacement une poussée SANS ALTERNATIVE.** La caisse (2,4) du 32 relève
  de « garder pour plus tard » et se retrouve classée « déplacée » à cause d'un coup forcé. Le
  correctif est local et rendrait le partage exact sur les cinq niveaux.
- [ ] **Chercher le prédicat statique de « ne gêne en rien »** : une caisse hors de tout trajet de
  livraison et hors des appuis nécessaires. `distanceLivraison` et `passages` fournissent déjà de quoi
  le construire. ⚠️ Ce serait un **guidage** (dé-prioriser les macros qui y touchent), pas un élagage —
  la forme sûre du §6.4(a).
- [ ] **Un seul banc est PUR : le 16** (quatre caisses à 81-90 % déplacées, rien au-dessus de 22 % sur
  place). ⚠️ **Correction du jour même** : j'avais écrit que le 14 et le 16 formaient « deux bancs
  disjoints ». **Faux, et l'utilisateur l'a relevé** — le 14 porte les DEUX notions : quatre caisses à
  80-97 % sur place ET `(2,3)→(2,9)` garée **65 %** de la partie après six cases de descente. La
  colonne « déplacée » du 14 n'était pas vide, seulement plus basse que l'autre, et j'ai lu
